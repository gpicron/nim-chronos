...
export
  results, asyncloop, asyncsync, asyncstream, tlsstream, chunkstream, boundstream,
  httptable, httpcommon, httpagent, httpbodyrw, multipart, httputils, uri, results
export SocketFlags
...
const
  ...
  HttpHeadersTimeout* = 120.seconds ## Timeout for receiving response headers (120 sec)
...
template checkClosed(reqresp: untyped): untyped =
  if reqresp.connection.state in
      {HttpClientConnectionState.Closing, HttpClientConnectionState.Closed}:
    let e = newHttpUseClosedError()
    reqresp.setError(e)
    raise e

template setTimestamp(conn: HttpClientConnectionRef, moment: Moment): untyped =
  if not (isNil(conn)):
    conn.timestamp = moment

template setTimestamp(reqresp: HttpClientRequestRef | HttpClientRequestRef): untyped =
...
template setDuration(reqresp: HttpClientRequestRef | HttpClientResponseRef): untyped =
  if not (isNil(reqresp)):
    let timestamp = Moment.now()
...
template isIdle(
    conn: HttpClientConnectionRef, timestamp: Moment, timeout: Duration
): bool =
...
proc getHttpAddress*(url: Uri, flags: HttpClientFlags = {}): HttpAddressResult =
  let
    scheme =
      if len(url.scheme) == 0:
    ...
    port =
      if len(url.port) == 0:
        case scheme
        of HttpClientScheme.NonSecure: 80'u16
        of HttpClientScheme.Secure: 443'u16
      else:
...
proc getUri*(address: HttpAddress): Uri =
  ...
  Uri(
    ...
    port: Base10.toString(address.port),
    path: address.path,
    query: address.query,
...
proc redirect*(
    session: HttpSessionRef, srcaddr: HttpAddress, uri: Uri
): HttpResult[HttpAddress] =
  ...
  if newuri.hostname != srcuri.hostname:
    ...
    ok(
      HttpAddress(
        id: id,
        scheme: scheme,
        hostname: newuri.hostname,
        port: port,
        path: newuri.path,
        query: newuri.query,
        anchor: newuri.anchor,
        username: newuri.username,
        password: newuri.password,
...
proc connect(
    session: HttpSessionRef, ha: HttpAddress
...
proc removeConnection(
    session: HttpSessionRef, conn: HttpClientConnectionRef
) {.async: (raises: []).} =
  let removeHost = block:
    var res = false
...
func connectionPoolEnabled(
    session: HttpSessionRef, flags: set[HttpClientRequestFlag]
): bool =
...
.} =
  ...
  if session.connectionPoolEnabled(flags):
    ...
    for connection in session.connections.getOrDefault(ha.id):
      if connection.isReady() and not (
        connection.isIdle(timestamp, session.idleTimeout)
      ):
...
proc releaseConnection(
    session: HttpSessionRef, connection: HttpClientConnectionRef
) {.async: (raises: []).} =
  ...
  if removeConnection:
    ...
    connection.flags.excl(
      {
        HttpClientConnectionFlag.Request, HttpClientConnectionFlag.Response,
...
proc releaseConnection(response: HttpClientResponseRef) {.async: (raises: []).} =
  let
...
proc prepareResponse(
    request: HttpClientRequestRef, data: openArray[byte]
  ...
  let connectionFlag = block:
    ...
    of HttpVersion11:
      ...
      if header == "close": false else: true
    of HttpVersion10:
  ...
  let res = HttpClientResponseRef(
    state: HttpReqRespState.Open,
    status: resp.code,
    address: request.address,
    requestMethod: request.meth,
    reason: resp.reason(data),
  ...
  let
    newConnectionAlways = HttpClientFlag.NewConnectionAlways in request.session.flags
    httpPipeline = HttpClientFlag.Http11Pipeline in request.session.flags
    closeConnection = HttpClientRequestFlag.CloseConnection in request.flags
...
proc getResponse(
  ...
  let bytesRead =
    try:
      await req.connection.reader
      .readUntil(addr req.headersBuffer[0], len(req.headersBuffer), HeadersMark)
      .wait(req.session.headersTimeout)
    except AsyncTimeoutError:
      raiseHttpReadError("Reading response headers timed out")
    except AsyncStreamError as exc:
      raiseHttpReadError("Could not read response headers, reason: " & $exc.msg)

  let response = prepareResponse(req, req.headersBuffer.toOpenArray(0, bytesRead - 1)).valueOr:
...
proc new*(
  ...
  let res = HttpClientRequestRef(
    state: HttpReqRespState.Ready,
    session: session,
    meth: meth,
    version: version,
    flags: flags,
    headers: HttpTable.init(headers),
    address: ha,
    bodyFlag: HttpClientBodyFlag.Custom,
    buffer: @body,
...
proc get*(
  ...
  HttpClientRequestRef.new(
    session, url, MethodGet, version, flags, maxResponseHeadersSize, headers
  )
...
proc get*(
  ...
  HttpClientRequestRef.new(
    session, ha, MethodGet, version, flags, maxResponseHeadersSize, headers
  )
...
proc post*(
  ...
  HttpClientRequestRef.new(
    session, url, MethodPost, version, flags, maxResponseHeadersSize, headers, body
  )
...
proc post*(
  ...
  HttpClientRequestRef.new(
    session, ha, MethodPost, version, flags, maxResponseHeadersSize, headers, body
  )
...
proc prepareRequest(request: HttpClientRequestRef): string =
  ...
  let entity = block:
    var res = if len(request.address.path) > 0: request.address.path else: "/"
...
proc send*(
  ...
  doAssert(
    request.state == HttpReqRespState.Ready, "Request's state is " & $request.state
  )
  let connection =
    try:
  ...
  try:
    ...
    request.setDuration()
    let error = newHttpWriteError("Could not send request headers, reason: " & $exc.msg)
...
proc open*(
    request: HttpClientRequestRef
): Future[HttpBodyWriter] {.async: (raises: [CancelledError, HttpError]).} =
  ...
  let writer =
    ...
    of HttpClientBodyFlag.Sized:
      let size = Base10.decode(uint64, request.headers.getString("content-length"))
...
proc finish*(
  ...
  doAssert(not (isNil(request.connection)), "Request missing connection instance")
  request.checkClosed()
  doAssert(
    request.state == HttpReqRespState.Open, "Request's state is " & $request.state
  )
  doAssert(
    request.connection.state == HttpClientConnectionState.RequestBodySending,
...
proc getBodyReader*(
...
): HttpBodyReader {.raises: [HttpUseClosedError].} =
  ...
  ## leaks.
  doAssert(not (isNil(response.connection)), "Response missing connection instance")
  response.checkClosed()
  doAssert(
    response.state == HttpReqRespState.Open, "Response's state is " & $response.state
  )
  doAssert(
    response.connection.state in {
...
proc getBodyBytes*(
    response: HttpClientResponseRef, nbytes: int
): Future[seq[byte]] {.async: (raises: [CancelledError, HttpError]).} =
...
proc consumeBody*(
    response: HttpClientResponseRef
): Future[int] {.async: (raises: [CancelledError, HttpError]).} =
  ...
  try:
    ...
    let error = newHttpReadError("Could not consume response, reason: " & $exc.msg)
    if not (isNil(reader)):
      await reader.closeWait()
    response.setError(error)
    raise error
...
proc redirect*(
    request: HttpClientRequestRef, ha: HttpAddress
): HttpResult[HttpClientRequestRef] =
  ...
  if redirectCount > request.session.maxRedirections:
    ...
    let headers = block:
      ...
      res
    var res = HttpClientRequestRef.new(
      request.session,
      ha,
      request.meth,
      request.version,
...
proc redirect*(
    request: HttpClientRequestRef, uri: Uri
): HttpResult[HttpClientRequestRef] =
  ## Create new request object using original request object ``request`` and
  ## redirected URL ``uri``.
  ##
...
proc fetch*(
  ...
  while true:
    try:
      ...
      var pending: seq[Future[void]]
      if not (isNil(response)):
        pending.add(closeWait(response))
      if not (isNil(request)):
        pending.add(closeWait(request))
      if not (isNil(redirect)):
        pending.add(closeWait(redirect))
      await noCancel(allFutures(pending))
      raise exc
    except HttpError as exc:
      var pending: seq[Future[void]]
...
proc getServerSentEvents*(
    response: HttpClientResponseRef, maxEventSize: int = -1
): Future[seq[ServerSentEvent]] {.async: (raises: [CancelledError, HttpError]).} =
      ...
      if data[i] in {CR, LF}:
        ...
        if (maxEventSize >= 0) and (len(buffer) > maxEventSize):
          if buffer[0] != COLON:
            ...
            error =
              newException(HttpReadLimitError, "Size of event exceeded maximum size")
            return (0, true)
          discardBuffer()
